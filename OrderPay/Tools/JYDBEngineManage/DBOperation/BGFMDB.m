 ...
 */
//lds add:2017.10.01
-(NSArray *)columnNames:(FMResultSet*)rs{
    int columnCount = sqlite3_column_count([rs.statement statement]);
    int columnIdx = 0;
    NSMutableArray* tempArrayM = [NSMutableArray array];
    for (columnIdx = 0; columnIdx < columnCount; columnIdx++){
        [tempArrayM addObject:[NSString stringWithUTF8String:sqlite3_column_name([rs.statement statement], columnIdx)]];
    }
...
-(void)ifIvarChangeForClass:(Class)cla ignoredKeys:(NSArray*)ignoredkeys{
    @autoreleasepool {
        ...
        [self executeDB:^(FMDatabase * _Nonnull db){
            ...
            // 2.遍历结果集
            if(rs.next){
                NSArray* columNames = [self columnNames:rs];
