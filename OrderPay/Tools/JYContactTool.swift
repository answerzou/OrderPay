...
//  EasyRepayment
//
//  Created by McIntosh on 16/1/25.
        ...
        if propertyValues != nil {
            ...
            for i in 0 ..< ABMultiValueGetCount(propertyValues) {
                ...
                case kABPersonAddressProperty :
                    ...
                    let addrNSDict:NSDictionary = value!.takeRetainedValue() as! NSDictionary
...
//        dispatch_semaphore_wait(authorizedSingal, DISPATCH_TIME_FOREVER)
        ...
        if sysAddressBookStatus == .denied || sysAddressBookStatus == .notDetermined {
            ABAddressBookRequestAccessWithCompletion(JYCommonObj.instance.addressBook){isAccess, errorsd in
                if isAccess
                {
                    getAddressBook()
                    //操作
                    JYCommonObj.saveContanctInfo(analyzeSysContacts( ABAddressBookCopyArrayOfAllPeople(JYCommonObj.instance.addressBook).takeRetainedValue() as NSArray ) as NSArray)
                }else{
    ...
    let sysContactsArr = ABAddressBookCopyArrayOfAllPeople(JYCommonObj.instance.addressBook).takeRetainedValue() as NSArray
...
func getAddressBook() {
    if JYCommonObj.instance.addressBook == nil {
        ...
        JYCommonObj.instance.addressBook = ABAddressBookCreateWithOptions(nil,&errorss)?.takeRetainedValue()
        ABAddressBookRegisterExternalChangeCallback(JYCommonObj.instance.addressBook, { aaa, ccc, context -> Void in
            //操作
            JYCommonObj.saveContanctInfo(analyzeSysContacts( ABAddressBookCopyArrayOfAllPeople(JYCommonObj.instance.addressBook).takeRetainedValue() as NSArray ) as NSArray)
        }, nil);
    }
